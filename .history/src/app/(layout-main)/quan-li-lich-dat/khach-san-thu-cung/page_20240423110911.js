...
export default function Page() {
  ...
  const handleSearch = (value, _e, info) => {
    if (value === "") {
      ...
      let searchData = data.filter((item) => {
        return item.id.trim() === value.trim();
      });
      setBookingData(searchData);
      setTotal(searchData.length);
    }
  ...
  return (
    <section className="section">
      <h1 className="section-heading">QUẢN LÝ LỊCH ĐẶT/ KHÁCH SẠN THÚ CƯNG</h1>
      <div
        className="filter"
        style={{
          marginBottom: "20px",
      ...
      >
        ...
        <Link href="/khach-san-thu-cun/them-lich-dat" className="add-new-btn">
          + Thêm lịch đặt
        </Link>
      </div>
      <div className="table">
          ...
          return (
            <div className="table-row" key={booking.id}>
              ...
              <div className="table-column">
                {booking.bookingStatus === "pending" && "Chờ tới"}
                {booking.bookingStatus === "wait" && "Chờ đón"}
                {booking.bookingStatus === "done" && "Thành công"}
